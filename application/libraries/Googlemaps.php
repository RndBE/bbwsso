...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyCdAD-PAuK58pHbGGHwCKQFo8tDsOd7dTQ'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $center						= "-7.358477, 110.185180";		// Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"
	...
	var $map_height					= "700px";					// The height of the map container. Any units (ie 'px') can be used. If no units are provided 'px' will be presumed
	...
	var $streetViewPanControl		= FALSE;						// If set to FALSE will hide the Pan control
...
//------------------------------------------------------------------------------------------ TAMBAHAN 
		...
		if ($marker['category_group'] != "")
		{
			$marker_output .= '
			var category_group = "'.$marker['category_group'].'";
			';
		}
	...
	function create_map()
	{
		...
		$this->output_js_contents .= '}
				'.$this->map_name.' = new google.maps.Map(document.getElementById("'.$this->map_div_id.'"), myOptions);
				  map.controls[google.maps.ControlPosition.TOP_RIGHT].push(document.getElementById("myID"));//--------------------------------------------------------------tambah legend
				';
		...
		//	gmarkers1.push(marker);
			 if (!markerGroups[markerOptions.category]) markerGroups[markerOptions.category] = [];
    		 markerGroups[markerOptions.category].push(marker);
    		  if (!markerStatus[markerOptions.category_group]) markerStatus[markerOptions.category_group] = [];
    		 markerStatus[markerOptions.category_group].push(marker);
    		
			return marker;
		}
		'; // tambahan gmarker1 push
		//
		// add filter
		$this->output_js_contents .= '
		  function filterMarkers(category) {
		    for (i = 0; i < markers_'.$this->map_name.'.length; i++) {
		        marker = gmarkers1[i];
		...
		';
		$this->output_js_contents .= '
		  function filterMarkers2(category) {
		  	var awlrCheck = document.getElementById("awlr").checked;
		  	var check = document.getElementById(category).checked;
		  	   ...
		  	   for (var i = 0; i < markerGroups[category].length; i++) {
				  ...
				  if(awlrCheck == true)
				 {
				 	 var x = document.getElementsByName("awlr");
					    var i;
		...
		$this->output_js_contents .= '
				function toggleGroup(category) {
					var kategori =  category.split("_");
				    for (var i = 0; i < markerStatus[category].length; i++) {
				        var marker = markerStatus[category][i];
				        if (!marker.getVisible()) {
				            marker.setVisible(true);
				        } else {
				            marker.setVisible(false);
