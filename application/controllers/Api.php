...
class Api extends CI_Controller
{
	...
	function live_data2()
	{
		...
		array_multisort(
			array_map('strtotime', array_column($data_1, 'waktu')),
			SORT_ASC,
			$data_1
		);
		$data_chart = [];
	...
	function dtakhir()
	{
		...
		if ($data_logger) {
			...
			foreach ($qparam->result() as $s) {
				...
				if ($s->nama_parameter == 'Illumination')
					$v /= 1000;
				if (!$is_perbaikan) {
	...
	function analisaperbulan()
	{
		...
		if ($tb_main) {
			...
			$data_psda = json_decode(file_get_contents('https://dpupesdm.monitoring4system.com/api/analisaperbulan2?idsensor=' . $idsensor . '&tanggal=' . $tanggal), true);
			if ($data_psda['data']) {
				$stts = 'sukses';
				$dataAnalisa = $data_psda;
			} else {
	...
	function analisaperrange()
	{
		...
		if ($tb_main) {
			...
			$data_psda = json_decode(file_get_contents('https://dpupesdm.monitoring4system.com/api/analisaperrange2?idsensor=' . $idsensor . '&dari=' . $awal . '&sampai=' . $akhir), true);
			if ($data_psda['data']) {
				$stts = 'sukses';
				$dataAnalisa = $data_psda;
			} else {
	...
	function analisapertahun()
	{
		...
		if ($tb_main) {
			...
			$data_psda = json_decode(file_get_contents('https://dpupesdm.monitoring4system.com/api/analisapertahun2?idsensor=' . $idsensor . '&tahun=' . $tanggal), true);
			if ($data_psda['data']) {
				$stts = 'sukses';
				$dataAnalisa = $data_psda;
			} else {
			...
			if (empty($data->elevasi)) {
				$data_informasi = array(
					...
					array(
						'nama' =>
							'Seri',
						'nilai' => $data->seri_logger
					),
					array(
						'nama' =>
							'Serial Number',
						'nilai' => $data->serial_number
					),
					array(
						'nama' =>
							'Sensor',
