...
class Awlr extends CI_Controller
{
	...
	# Halaman kelengkapan data per menit + tarik ulang data dari logger.
	# Hanya untuk pos yang loggernya sudah mendukung perintah tarik ulang —
	# tambahkan id_logger-nya ke daftar ini kalau ada pos baru.

	private $rekon_logger = array(
		'10051',    // Pos AWLR Butuh
	);

	private function _boleh_rekonsiliasi()
	{
		$kode = (string) $this->session->userdata('idlogger');

		return $kode !== '' && in_array($kode, $this->rekon_logger, TRUE);
	}

	# Tabel data logger ditentukan t_logger.tabel_main (awlr, awlr2, … awlr5),
	# bukan sesi 'tabel' yang datang dari query string.
	private function _tabel_rekonsiliasi()
	{
		$row = $this->db
			->select('tabel_main')
			->where('id_logger', $this->session->userdata('idlogger'))
			->get('t_logger')
			->row();
	...
	{
		if (!$this->_boleh_rekonsiliasi()) {
			show_404();
			return;
		}

		$data['konten'] = 'konten/back/awlr/rekonsiliasi_awlr';
		$this->load->view('template_admin/site', $data);
	}
	...
	function rekonsiliasi_menit()
	{
		...
		$tabel = $this->_tabel_rekonsiliasi();
		if (!$tabel) {
			$this->output
		...
		$this->db->where('code_logger', $this->session->userdata('idlogger'));
