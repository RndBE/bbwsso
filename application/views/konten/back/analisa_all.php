...
<style>
	...
	.datepicker-years thead th.datepicker-switch {
		flex: 1;
		text-align: center;
		font-weight: 600;
		user-select: none;
	...
	/* ✅ Scroll isi tahun kalau banyak */
	.datepicker-years table {
		max-height: 250px;
		overflow-y: auto;
		display: block;
	}
	@media only screen and (max-width:576px){#target{display:none}}
	.btn-info{background-color:#303481}
	.btn-info:hover{text-decoration:none;background-color:#000342;border-color:#000342}
	.circle{width:12px;height:12px;border-radius:50%;box-shadow:0 0 1px 1px #0000001a}
	.pulse-brown{background:#876a2f;animation:pulse-animation-brown 2s infinite}
	@keyframes pulse-animation-brown{0%{box-shadow:0 0 0 0 #876a2f}100%{box-shadow:0 0 0 15px rgba(0,0,0,0)}}
</style>
<?php
...
if($mode==='hari' && $typegraf==='column'){
	...
	elseif($akumRaw<5 && $akumRaw>=0.1){$img='kotak-cyan.png';$txtCls='text-white';}
	elseif($akumRaw<20){$img='kotak-nila.png';$txtCls='text-white';}
	elseif($akumRaw<50){$img='kotak-kuning.png';$txtCls='';}
	elseif($akumRaw<100){$img='kotak-oranye.png';$txtCls='text-white';}
...
<div class="container-md">
	<div class="page-header d-print-none">
		<div class="row g-3 align-items-center">
			...
			<div class="col-auto">
				<?php if($isPerbaikan){?><div class="circle pulse-brown mx-3"></div><?php }else{?>
				<span class="status-indicator status-<?= htmlspecialchars($temp_data['color'],ENT_QUOTES) ?> status-indicator-animated"><span class="status-indicator-circle"></span><span class="status-indicator-circle"></span><span class="status-indicator-circle"></span></span>
				<?php } ?>
			</div>
				...
				<div class="text-muted">
					<ul class="list-inline list-inline-dots mb-0">
						<li class="list-inline-item">
							<?php if($isPerbaikan){?>
							<span style="color:#876a2f"><?= htmlspecialchars($temp_data['status_logger'],ENT_QUOTES) ?></span>
							<?php }else{?>
							<span class="text-<?= htmlspecialchars($temp_data['color'],ENT_QUOTES) ?>"><?= htmlspecialchars($temp_data['status_logger'],ENT_QUOTES) ?></span>
							<?php } ?>
						</li>
					</ul>
				</div>
			...
			<div class="col-12 col-md">
				<div class="row g-2 align-items-center justify-content-end">
					...
					<div class="col-8 col-md-auto d-flex">
						...
						<button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#skema_perangkat">
							<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icons-tabler-outline icon-tabler-schema" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 2h5v4h-5z" /><path d="M15 10h5v4h-5z" /><path d="M5 18h5v4h-5z" /><path d="M5 10h5v4h-5z" /><path d="M10 12h5" /><path d="M7.5 6v4" /><path d="M7.5 14v4" /></svg>
							Skema Perangkat
						</button>
						<?php } ?>
			...
			<div class="col-md-3 col-xxl-2" id="target">
				<div class="row row-cards">
							...
							<div class="card-body">
								...
								<div class="h3 m-0">
									<form action="<?= base_url() ?>analisa/set_token" method="post" id="form-pos">
										...
										<select name="id_logger" class="form-select" onchange="validate_form()" id="select-pos">
										  ...
										  $log_select = $merge[0];
											...
											<option value="<?= $mnpos->idLogger ?>" <?= ($idLogger==$log_select)?'selected':'' ?>><?= str_replace('_',' ',$mnpos->namaPos) ?></option>
											<?php endforeach ?>
										</select>
									</form>
								</div>
						...
						<div class="card">
							<div class="card-body">
								...
								<div class="h3 m-0">
									...
									<input value="<?= $token ?>" name="token" class="d-none"/>
									<select name="id_param" class="form-select" onchange="this.form.submit()" id="select-parameter">
										<option value="">Pilih Parameter</option>
										<?php foreach($pilih_parameter as $mnparameter):?>
										<option value="<?= $mnparameter->idParameter ?>" <?= ($data_sensor->idParam==$mnparameter->idParameter)?'selected':'' ?>><?= str_replace('_',' ',$mnparameter->namaParameter)?></option>
										<?php endforeach ?>
									</select>
									<?= form_close(); ?>
								</div>
									...
									<div class="row">
										<div class="col-12">
											...
											<div class="input-icon">
												<input class="form-control" name="pada" id="dptanggal" placeholder="Pilih Tanggal" value="<?= $tglPada ?>" autocomplete="off" required/>
											...
											<div class="input-icon">
												<input type="month" class="form-control" name="pada" value="<?= $tglPada ?>" autocomplete="off" required/>
											</div>
											<?php }elseif($mode==='tahun'){?>
											<div class="input-icon">
												<input class="form-control" name="pada" id="yearPicker" placeholder="Pilih Tahun" value="<?= $tglPada ?>" autocomplete="off" required/>
												<span class="input-icon-addon"><svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><rect x="4" y="5" width="16" height="16" rx="2" /><line x1="16" y1="3" x2="16" y2="7" /><line x1="8" y1="3" x2="8" y2="7" /><line x1="4" y1="11" x2="20" y2="11" /><line x1="11" y1="15" x2="12" y2="15" /><line x1="12" y1="15" x2="12" y2="18" /></svg></span>
											</div>
											<?php }else{?>
											<div class="row">
												<div class="col-12">
													...
													<div class="input-icon">
														<input class="form-control" name="dari" id="dpdari" placeholder="Dari" value="<?= $tglDari ?>" autocomplete="off" required/>
														<span class="input-icon-addon"><svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><rect x="4" y="5" width="16" height="16" rx="2" /><line x1="16" y1="3" x2="16" y2="7" /><line x1="8" y1="3" x2="8" y2="7" /><line x1="4" y1="11" x2="20" y2="11" /><line x1="11" y1="15" x2="12" y2="15" /><line x1="12" y1="15" x2="12" y2="18" /></svg></span>
													</div>
												</div>
												<div class="col-12">
													...
													<div class="input-icon">
														<input class="form-control" name="sampai" id="dpsampai" placeholder="Sampai" value="<?= $tglSampai ?>" autocomplete="off" required/>
														<span class="input-icon-addon"><svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><rect x="4" y="5" width="16" height="16" rx="2" /><line x1="16" y1="3" x2="16" y2="7" /><line x1="8" y1="3" x2="8" y2="7" /><line x1="4" y1="11" x2="20" y2="11" /><line x1="11" y1="15" x2="12" y2="15" /><line x1="12" y1="15" x2="12" y2="18" /></svg></span>
													</div>
												</div>
											</div>
											<?php } ?>
											<div class="form-footer mt-3"><input type="submit" class="btn btn-info w-100" value="Tampil"/></div>
										</div>
									</div>
									<?= form_close() ?>
								</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-body pt-3">
								...
								<?= form_open('analisa/set_token') ?>
								<input value="<?= $token ?>" name="token" class="d-none"/>
								<label class="form-check"><input class="form-check-input" type="radio" name="mode" value="hari" onclick="this.form.submit()" <?= $mode==='hari'?'checked':'' ?> /><span class="form-check-label">Hari</span></label>
								<label class="form-check"><input class="form-check-input" type="radio" name="mode" value="bulan" onclick="this.form.submit()" <?= $mode==='bulan'?'checked':'' ?> /><span class="form-check-label">Bulan</span></label>
								<label class="form-check"><input class="form-check-input" type="radio" name="mode" value="tahun" onclick="this.form.submit()" <?= $mode==='tahun'?'checked':'' ?> /><span class="form-check-label">Tahun</span></label>
								<label class="form-check mb-0"><input class="form-check-input" type="radio" name="mode" value="range" onclick="this.form.submit()" <?= $mode==='range'?'checked':'' ?> /><span class="form-check-label">Rentang Waktu</span></label>
								<?= form_close() ?>
							</div>
						</div>
					</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-body">
								<button onclick="ExportToExcel('xlsx')" class="btn btn-outline-success w-100"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-file-spreadsheet" width="40" height="40" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M14 3v4a1 1 0 0 0 1 1h4"></path><path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z"></path><path d="M8 11h8v7h-8z"></path><path d="M8 15h8"></path><path d="M11 11v7"></path></svg>Download Excel</button>
								<?php if($data_op){?>
								<button data-bs-toggle="modal" data-bs-target="#exampleModal" class="btn btn-outline-primary w-100 mt-3"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-history"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 8l0 4l2 2" /><path d="M3.05 11a9 9 0 1 1 .5 4m-.5 5v-5h5" /></svg>Riwayat O & P</button>
								<?php } ?>
							</div>
						</div>
					</div>
				...
				<div class="row row-cards">
						...
						<div class="card card-sm h-100">
							<div class="card-body d-flex align-items-center">
								<div class="row align-items-center">
									<div class="col-auto">
										<span class="form-control w-100 rounded <?= $txtCls ?>" style="background-image:url(<?= base_url() ?>pin_marker/<?= $img ?>);background-repeat:no-repeat;background-position:center center;">
											<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-cloud-rain" width="50" height="50" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M7 18a4.6 4.4 0 0 1 0 -9a5 4.5 0 0 1 11 2h1a3.5 3.5 0 0 1 0 7"></path><path d="M11 13v2m0 3v2m4 -5v2m0 3v2"></path></svg>
										</span>
									</div>
									<div class="col">
										<div class="font-weight-medium"><div class="subheader">Akumulasi Curah Hujan tanggal <?= $tglPada ?></div></div>
					...
					<div class="col-sm-9 col-lg-9 mb-3">
						<div class="card card-sm h-100">
							<div class="card-body d-flex align-items-center">
								<div class="row w-100">
									<div class="col-auto d-flex align-items-center justify-content-xl-between"><h4 class="mb-0 mt-0">Keterangan</h4><h4 class="mb-0 mt-0">:</h4></div>
									<div class="col-xl-auto mt-2 mt-xxl-0">
										<div class="row gx-2 align-items-center">
											<?php foreach($legend as $it){?>
											<div class="col-6 col-sm-auto">
												<div class="row gx-2 mt-2 mt-md-0">
													<div class="col-auto d-flex align-items-center"><div class="rounded border border-dark" style="background-color:<?= $it[0] ?>;height:35px;width:35px"></div></div>
													<div class="col-auto"><h5 class="mb-0"><?= $it[1] ?></h5><h5 class="mb-0 text-muted"><?= $it[2] ?></h5></div>
												</div>
											</div>
											<?php } ?>
										</div>
						...
						<div class="card card-sm h-100">
							<div class="card-body d-flex align-items-center py-2">
								<div class="row align-items-center">
									...
									<div class="col">
										<div class="font-weight-medium"><div class="subheader">Akumulasi Curah Hujan <br> <?= $tglDari ?> - <?= $tglSampai ?></div></div>
										<div class="h1 mb-0 me-2 mt-0"><?= $akumStr ?> mm</div>
									</div>
								</div>
							</div>
								...
								<div class="w-100 mt-3 card">
									...
									<div class="table-responsive">
										<table class="table mb-0 table-bordered table-sm" id="tbl_exporttable_to_xls">
											<thead>
												<tr><th colspan="4"><h5 class="mb-0 fw-bold"><?= str_replace('_',' ',$data_sensor->namaSensor) ?> <?= $titleTable ?></h5></th></tr>
												<tr><th class="d-none"><h5 class="mb-0 fw-bold"><?= $titleTable ?></h5></th></tr>
												<tr>
													<th>Waktu</th>
													<th><?= str_replace('_',' ',$data_sensor->namaSensor)?></th>
													<?php if($typegraf!='column'){?><th>Minimal</th><th>Maksimal</th><?php } ?>
												</tr>
											</thead>
											<tbody>
												...
												<tr>
													<td><?= $dt->waktu ?></td>
													<td><?= $dt->dta.' '.$satuan ?></td>
													<?php if($typegraf!='column'){?><td><?= $dt->min.' '.$satuan ?></td><td><?= $dt->max.' '.$satuan ?></td><?php } ?>
												</tr>
												<?php endforeach;?>
											</tbody>
										</table>
									...
									<div class="offcanvas-body">
										<div>
											<table class="table table-sm table-borderless">
												<tbody>
													<tr><td class="fw-bold">ID Logger</td><td class="text-end"><?= $informasi->logger_id ?></td></tr>
													<tr><td class="fw-bold">Seri Logger</td><td class="text-end"><?= isset($informasi->seri_logger) ? $informasi->seri_logger:$informasi->seri ?></td></tr>
													<tr><td class="fw-bold">Sensor</td><td class="text-end"><?= $informasi->sensor ?></td></tr>
													<tr><td class="fw-bold">Serial Number</td><td class="text-end"><?= $informasi->serial_number ?></td></tr>
													<tr><td class="fw-bold">Nomor Seluler</td><td class="text-end"><?= $informasi->nosell ?></td></tr>
													<?php if($this->uri->segment(1)=='awlr'){?><tr><td class="fw-bold">Elevasi</td><td class="text-end"><?= $informasi->sensor ?></td></tr><?php } ?>
													<tr><td class="fw-bold">Nama Penjaga</td><td class="text-end"><?= $informasi->nama_pic==''?'-':$informasi->nama_pic ?></td></tr>
													<tr><td class="fw-bold">Nomor Penjaga</td><td class="text-end"><?= $informasi->no_pic==''?'-':$informasi->no_pic ?></td></tr>
												</tbody>
											</table>
											<?php if($foto_pos){?><h3 class="text-center">Foto Pos</h3><?php foreach($foto_pos as $vl){?><img src="https://bbws.beacontelemetry.com/image/foto_pos/<?= $vl['url_foto']?>" class="img-fluid mb-2 rounded"/><?php }} ?>
											<?php if($idLogger==='10247'){?>
											<h3 class="text-center mb-1">Titik BM Pos Sungai Bogowonto</h3><img src="https://bbwsso.monitoring4system.com/image/SB.jpeg" class="img-fluid mb-2 rounded"/>
											<h3 class="text-center mt-3 mb-1">Titik BM Pos Carik Barat</h3><img src="https://bbwsso.monitoring4system.com/image/CB.jpeg" class="img-fluid mb-2 rounded"/>
											<?php } ?>
										</div>
									</div>
								</div>
...
<div class="modal fade" id="skema_perangkat" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-lg">
		<div class="modal-content">
			<div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">Skema Perangkat</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
			<div class="modal-body p-3 text-center d-flex justify-content-center">
				<?php if($idLogger==='10249'){?><img src="<?= base_url() ?>image/afmr.jpg" class="img-fluid w-100"/><?php }else{?><img src="<?= base_url() ?>image/awlr_bogowonto.png" class="img-fluid w-100"/><?php } ?>
			</div>
		</div>
	</div>
</div>
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">Riwayat Operasional dan Perawatan</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
				...
				<div class="accordion" id="accordionExample">
					...
					<div class="accordion-item">
						<h2 class="accordion-header py-0">
							<button class="accordion-button py-2 border-bottom" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $k ?>" aria-expanded="false" aria-controls="collapse<?= $k ?>"><?= $dp['tanggal'] ?></button>
						</h2>
						<div id="collapse<?= $k ?>" class="accordion-collapse" data-bs-parent="#accordionExample">
							<div class="accordion-body py-2 text-end">
								<a class="btn btn-success" href="https://bbwsso.monitoring4system.com/unduh/laporan_op/<?= $dp['file'] ?>" target="_blank">
...
<script>
	$('#yearPicker').datepicker({
		format: "yyyy",          // tampil hanya tahun
		viewMode: "years",       // tampilan awal: daftar tahun
		minViewMode: "years",    // batasi agar hanya bisa pilih tahun
		autoclose: true,         // langsung tutup setelah pilih
		todayHighlight: true
	});
	...
	Highcharts.chart('analisa',{
		...
		exporting:{
			...
			showTable:false,
			<?php if($this->session->userdata('username')=='serayu_opak'){echo 'enabled:false';} ?>
		},
